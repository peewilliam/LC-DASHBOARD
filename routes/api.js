...
router.get('/status', async (req, res, next) => {
  const result = await executeQuery(`
    SELECT
      CAST(NOMSTATUSFRE AS VARCHAR(999) CHARACTER SET ISO8859_1) AS NOMSTATUSFRE,
      COLOR
    FROM
      TABSTATUSFRE
    WHERE
...
router.get('/pedidos', async (req, res, next) => {
  ...
  const result = await executeQuery(`
    SELECT
      PED.NOMOVTRA,
      CAST(CLI.NOMCLI AS VARCHAR(999) CHARACTER SET ISO8859_1) AS CLIENTE,
      COALESCE(PED.CONTAINER, 'CARGA SOLTA') AS CONTAINER,
      CAST(STF.NOMSTATUSFRE AS VARCHAR(999) CHARACTER SET ISO8859_1) AS NOMSTATUSFRE,
      FLW.DATA AS DATA_FOLLOW,

      CAST(COL.NOMCLI AS VARCHAR(999) CHARACTER SET ISO8859_1) AS COLETA,
      CCOL.NOMCID AS CID_COLETA,
      CCOL.UFCID AS UF_COLETA,

      CAST(DEST.NOMCLI AS VARCHAR(999) CHARACTER SET ISO8859_1) AS DESTINATARIO,
      CDEST.NOMCID AS CID_DESTINATARIO,
      CDEST.UFCID AS UF_DESTINATARIO,

      CAST(DISTR.NOMCLI AS VARCHAR(999) CHARACTER SET ISO8859_1) AS ENTREGA,
      CDISTR.NOMCID AS CID_ENTREGA,
      CDISTR.UFCID AS UF_ENTREGA,

      CAST(REM.NOMCLI AS VARCHAR(999) CHARACTER SET ISO8859_1) AS REMETENTE,
...
router.get('/pedidos-modal/:nomovtra', async (req, res, next) => {
  ...
  const result = await executeQuery(`
    SELECT
      PED.NOMOVTRA,
      CAST(CLI.NOMCLI AS VARCHAR(999) CHARACTER SET ISO8859_1) AS NOMCLI,
    ...
    FROM
      TABMOVTRA PED
    LEFT OUTER JOIN
      V_PEDIDO_FINANCEIRO VPF ON VPF.NOMOVTRA = PED.NOMOVTRA
    LEFT OUTER JOIN
      TABCLI CLI ON CLI.NOCLI = PED.NOCLI
    LEFT OUTER JOIN
